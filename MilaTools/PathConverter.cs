...
namespace MilaTools
{
    public static class PathConverter
    {
        ...
        /// </summary>
        /// <param name="windowsPath">Windows-style path (e.g., C:\Users\Username\Desktop\file.txt).</param>
        /// <returns>WSL path (e.g., /mnt/c/Users/Username/Desktop/file.txt).</returns>
        public static string WindowsToWSL(string windowsPath)
        {
            ...
            // Process drive letter (e.g., C:\)
            if (fullPath.Length > 2 && char.IsLetter(fullPath[0]) && fullPath[1] == ':' && fullPath[2] == '/')
            {
                string driveLetter = char.ToLower(fullPath[0]).ToString();
                string wslPath = "/mnt/" + driveLetter + fullPath.Substring(2);
                return wslPath;
            }
            return fullPath;
        }
        ...
        /// </summary>
        /// <param name="wslPath">WSL path (e.g., /mnt/c/Users/Username/Desktop/file.txt).</param>
        /// <returns>Windows-style path (e.g., C:\Users\Username\Desktop\file.txt).</returns>
        public static string WSLToWindows(string wslPath)
        {
            ...
            string path = wslPath.Replace('/', '\\');
            if (path.StartsWith("\\mnt\\") && path.Length > 6 && char.IsLetter(path[5]))
            {
                string driveLetter = char.ToUpper(path[5]).ToString();
                string winPath = driveLetter + ":\\" + path.Substring(7);
                return winPath;
            }
            return path;
        }
